...
"""
This file includes functions to convert a list of ADCs into a model usable by
the optimizer. The top-level function is build_model.

Modeling is done by:
    1. Receiving an input list of ADCs
    2. Removing all ADCs excluded by the constraints list
    3. Using multivariate regression to scale ADC area/energy with respect to
    frequency, resolution, and technology node.
    4. Generating an approximate Pareto set of minimum energy-area ADCs
    5. Generating a real Pareto set including enough ADCs for all user
    constraints
    6. Lastly, use regression one more time to create a tradeoff line among
    the Pareto points
"""
...
def get_approx_pareto_group(data: pd.DataFrame, constraints: Dict
...
def get_pareto_group(data: pd.DataFrame, constraints: Dict
                     ) -> pd.DataFrame or None:
    """
    ...
    approx = get_approx_pareto_group(data, constraints)
...
def model_from_pareto(pareto_group: pd.DataFrame, param_fit: Dict) -> Dict:
...
def build_model(source_data: pd.DataFrame, constraints: Dict, output_file: str):
    ...
    # Get pareto group
    pareto = get_pareto_group(logscale_data, constraints)
...
if __name__ == '__main__':
    with open('adc_data/constraints.yaml') as f:
